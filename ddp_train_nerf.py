...
import time
from data_loader_split import load_data_split, parse_txt
...
def sample_pdf(bins, weights, N_samples, det=False):
    ...
    if det:
        ...
        u = u.view([1]*len(dots_sh) + [N_samples]
                   ).expand(dots_sh + [N_samples, ])   # [..., N_samples]
    else:
        sh = dots_sh + [N_samples]
        u = torch.rand(*sh, device=bins.device) * (max_cdf -
                                                   min_cdf) + min_cdf        # [..., N_samples]

    # Invert CDF
    # [..., N_samples, 1] >= [..., 1, M] ----> [..., N_samples, M] ----> [..., N_samples,]
    above_inds = torch.sum(
        u.unsqueeze(-1) >= cdf[..., :M].unsqueeze(-2), dim=-1).long()

    # random sample inside each bin
    below_inds = torch.clamp(above_inds-1, min=0)
    inds_g = torch.stack((below_inds, above_inds), dim=-
                         1)     # [..., N_samples, 2]

    # [..., N_samples, M+1]
    cdf = cdf.unsqueeze(-2).expand(dots_sh + [N_samples, M+1])
    # [..., N_samples, 2]
    cdf_g = torch.gather(input=cdf, dim=-1, index=inds_g)

    # [..., N_samples, M+1]
    bins = bins.unsqueeze(-2).expand(dots_sh + [N_samples, M+1])
    # [..., N_samples, 2]
...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size):
    ...
    rank_split_sizes[-1] = ray_batch['ray_d'].shape[0] - \
        sum(rank_split_sizes[:-1])
        ...
        for m in range(models['cascade_level']):
            ...
            if m == 0:
                ...
                fg_depth = torch.stack(
                    [fg_near_depth + i * step for i in range(N_samples)], dim=-1)  # [..., N_samples]
    ...
    if rank == 0:
        ...
        for m in range(len(ret_merge_chunk)):
            for key in ret_merge_chunk[m]:
                ...
                ret_merge_rank[m][key] = [torch.zeros(
                    *[size, ]+sh, dtype=torch.float32) for size in rank_split_sizes]
...
def log_view_to_wandb(log_data, gt_img, mask, prefix=''):
    rgb_im = img_HWC2CHW(torch.from_numpy(gt_img))
    wandb.log({prefix + 'rgb_groundtruth': wandb.Image(rgb_im)})

    for m in range(len(log_data)):
        rgb_im = img_HWC2CHW(log_data[m]['rgb'])
        # just in case diffuse+specular>1
        rgb_im = torch.clamp(rgb_im, min=0., max=1.)
        wandb.log({prefix + f'level_{m}/rgb': wandb.Image(rgb_im)})

        rgb_im = img_HWC2CHW(log_data[m]['fg_rgb'])
        # just in case diffuse+specular>1
        rgb_im = torch.clamp(rgb_im, min=0., max=1.)
        wandb.log({prefix + f'level_{m}/foreground_rgb': wandb.Image(rgb_im)})
        depth = log_data[m]['fg_depth']
        depth_im = img_HWC2CHW(colorize(depth, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        wandb.log({prefix + f'level_{m}/foreground_depth': wandb.Image(depth_im)})

        rgb_im = img_HWC2CHW(log_data[m]['bg_rgb'])
        # just in case diffuse+specular>1
        rgb_im = torch.clamp(rgb_im, min=0., max=1.)
        wandb.log({prefix + f'level_{m}/background_rgb': wandb.Image(rgb_im)})
        depth = log_data[m]['bg_depth']
        depth_im = img_HWC2CHW(colorize(depth, cmap_name='jet', append_cbar=True,
                                        mask=mask))
        wandb.log({prefix + f'level_{m}/background_depth': wandb.Image(depth_im)})
        ...
        # writer.add_image(prefix + 'level_{}/bg_lambda'.format(m), bg_lambda_im, global_step)
        wandb.log(
            {prefix + f'level_{m}/background_lambda': wandb.Image(bg_lambda_im)})
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        net = NerfNetWithAutoExpo(
            args, optim_autoexpo=args.optim_autoexpo, img_names=img_names).to(rank)
        net = DDP(net, device_ids=[
                  rank], output_device=rank, find_unused_parameters=True)
        # net = DDP(net, device_ids=[rank], output_device=rank)
        optim = torch.optim.Adam(net.parameters(), lr=args.lrate)
        models[f'net_{m}'] = net
        models[f'optim_{m}'] = optim
...
def ddp_train_nerf(rank, args):
    ...
    [H, W], intrinsics_files, pose_files, img_files, _, _, _ = load_data_split(
        args.datadir, args.scene, split='train', try_load_min_depth=args.load_min_depth)
    _, val_intrinsics_files, val_pose_files, val_img_files, _, _, _ = load_data_split(args.datadir, args.scene, split='validation',
    ...
    if rank == 0:
        wandb.init(project="fyusion-nerfplusplus",
                   tags=["nerfplusplus"], config=args)
        wandb.watch(models["net_0"])
        wandb.watch(models["net_1"])
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        ray_sampler = RaySamplerSingleImage(H=H, W=W, intrinsics=intrinsics, c2w=pose,
                                            img_path=img_files[i],
                                            resolution_level=8)
        ray_batch = ray_sampler.random_sample(args.N_rand, center_crop=False)
        for key in ray_batch:
            if torch.is_tensor(ray_batch[key]):
                ray_batch[key] = ray_batch[key].to(rank)

        # Create progress variable to linearly scale up higher frequencies
        progress = 0.0
        if global_step < 40000:
            progress = 0.0
        elif global_step >= 40000 and global_step < 80000:
            progress = (global_step - 40000) / 40000.0
        else:
            progress = 1.0
        scalars_to_log['progress'] = progress

        # forward and backward
        dots_sh = list(ray_batch['ray_d'].shape[:-1])  # number of rays
        all_rets = []                                  # results on different cascade levels
        for m in range(models['cascade_level']):
            optim = models[f'optim_{m}']
            net = models[f'net_{m}']
            ...
            ret = net(ray_batch['ray_o'], ray_batch['ray_d'], fg_far_depth, fg_depth,
                      bg_depth, img_name=ray_batch['img_name'], progress=progress)
            all_rets.append(ret)

            rgb_gt = ray_batch['rgb'].to(rank)
            if 'autoexpo' in ret:
                scale, shift = ret['autoexpo']
                scalars_to_log[f'level_{m}/autoexpo_scale'] = scale.item()
                scalars_to_log[f'level_{m}/autoexpo_shift'] = shift.item()
                ...
                loss = rgb_loss
            scalars_to_log[f'level_{m}/loss'] = rgb_loss.item()
            scalars_to_log[f'level_{m}/pnsr'] = mse2psnr(rgb_loss.item())
            loss.backward()
            optim.step()
        ...
        if global_step % args.i_img == 0 or global_step == start+1:
            ...
            time0 = time.time()
            idx = what_val_to_log % len(val_img_files)
            val_intrinsics = parse_txt(val_intrinsics_files[idx])
            val_pose = parse_txt(val_pose_files[idx])
            val_ray_sampler = RaySamplerSingleImage(H=H, W=W, intrinsics=val_intrinsics, c2w=val_pose,
                                            img_path=val_img_files[idx],
                                            resolution_level=8)
            log_data = render_single_image(
                rank, args.world_size, models, val_ray_sampler, args.chunk_size)
            what_val_to_log += 1
            dt = time.time() - time0
            if rank == 0:    # only main process should do this
                ...
                log_view_to_wandb(
                    log_data, gt_img=val_ray_sampler.get_img(), mask=None, prefix='val/')

            time0 = time.time()
            idx = what_train_to_log % len(img_files)
            log_data = render_single_image(
                rank, args.world_size, models, ray_sampler, args.chunk_size)
            what_train_to_log += 1
            dt = time.time() - time0
            if rank == 0:   # only main process should do this
                logger.info(
                ...
                log_view_to_wandb(
                    log_data, gt_img=ray_sampler.get_img(), mask=None, prefix='train/')
...
def config_parser():
    ...
    parser.add_argument("--lambda_autoexpo", type=float,
                        default=1., help='regularization weight for autoexposure')
    ...
    parser.add_argument("--load_min_depth", action='store_true',
